...
AUTHORIZED_SCOPES_PATH = HERMES_HOME / "google_oauth_scopes.json"
...
SCOPES = list(dict.fromkeys(scope for scopes in SERVICE_SCOPES.values() for scope in scopes))


def scopes_for_services(raw: str | None) -> list[str]:
    ...
    if unknown:
        ...
        print(f"Valid services: all,{','.join(SERVICE_SCOPES)}")
        sys.exit(2)
    return [scope for name in names for scope in SERVICE_SCOPES[name]]
...
def _missing_scopes_from_payload(payload: dict) -> list[str]:
    ...
    required = set(SCOPES)
    if AUTHORIZED_SCOPES_PATH.exists():
        try:
            configured = json.loads(AUTHORIZED_SCOPES_PATH.read_text())
            if isinstance(configured, list) and configured:
                required = {str(s).strip() for s in configured if str(s).strip()}
        except Exception:
            pass
...
def exchange_auth_code(code: str):
    ...
    AUTHORIZED_SCOPES_PATH.write_text(json.dumps(requested_scopes, indent=2))
...
def revoke():
    ...
    PENDING_AUTH_PATH.unlink(missing_ok=True)
    AUTHORIZED_SCOPES_PATH.unlink(missing_ok=True)
...
def main():
    ...
    parser.add_argument(
        "--services",
        default="all",
        help="Comma-separated services: email,calendar,drive,contacts,sheets,docs (default: all)",
    )
    ...
    if getattr(args, "check_live", False):
        ...
        get_auth_url(scopes_for_services(args.services))
    elif args.auth_code:
